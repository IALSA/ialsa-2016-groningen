...
# ---- declare-variables  ----------------------------------------
dv_name <- "smoked_ever"
dv_label <- "P(Smoked Ever)"
dv_label_odds <- "Odds(Smoked Ever)"

ds2 <- ds %>% 
  dplyr::select_("id", "study_name", "smoked_ever", "age_in_years", "female", "marital", "educ3","poor_health") %>%
...
g <- graph_logistic_main(ds_replicated)
g
