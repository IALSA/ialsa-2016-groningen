...
read_chunk("./models/exercise-report/detailed-model-report.R") 
```

This document reports the details of each model estimated in the Groningen Harmonization Exercise. 
...
```{r make-results-table, echo=F, results='hold', message=FALSE}
```
...
```{r local-results, echo=F, eval=TRUE, results='asis'}
```
...
```{r pooled-results, echo=F, eval=TRUE, results='asis'}
```
