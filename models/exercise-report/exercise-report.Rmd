...
Each of the following models (`A`, `B`, `AA`, and `BB`) are fitted to the data from each study separately. When fitted to the pooled data, an aditional predictor, `study_name` is added after the intercept. 
...
|marital status   |marital      |marital        |marital        |marital        |
|health           |             |poor_health    |               |poor_health    |
|physical activity|             |sedentary      |               |sedentary      |
|employment status|             |current_work   |               |current_work   |
...
# Separate
```{r local-results, echo=FALSE, results='hold'}
```

## ALSA 
```{r local-results-alsa, echo=FALSE,  results='asis'}
```

## LBLS 
```{r local-results-lbsl, echo=FALSE,  results='asis'}
```

## SATSA 
```{r local-results-satsa, echo=FALSE, results='asis'}
```

## SHARE 
```{r local-results-share, echo=FALSE,  results='asis'}
```

## TILDA 
```{r local-results-tilda, echo=FALSE, results='asis'}
```
