...
```{r model-A-global-1, echo=TRUE, eval=TRUE}
```

Compute odds-ratio 
```{r model-A-global-2, echo=TRUE, eval=TRUE}
```

Find the best subset
```{r model-A-global-3, echo=TRUE, eval=TRUE}
```

Compute odds-ratio 
```{r model-A-global-4, echo=TRUE, eval=TRUE}
```

### Separate 
`study_name` defines clusters for separate estimation. 
```{r model-A-local-1, echo=TRUE, eval=TRUE}
```


```{r model-A-local-2, echo=FALSE, eval=TRUE, results='markup'}
```
