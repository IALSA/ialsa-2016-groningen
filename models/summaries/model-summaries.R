...
estimate_pooled_model <- function(ds, predictors){
  eq_formula <- as.formula(paste0(pooled_stem, predictors))
  model <- glm(eq_formula, data = ds, family = binomial(link="logit")) 
  
  ds2$dv_p <- predict(model)
  ...
  predicted_global                  <- predict(model, newdata=ds_predicted_global, se.fit=TRUE)
...
estimate_local_models <- function(ds, predictors){
  eq_local <- as.formula(paste0(local_stem, predictors))
  model <- glm(eq_local, data = ds, family = binomial(link="logit")) 
  ...
  for( study_name_ in dto[["studyName"]] ) {
    ...
    predicted_study           <- predict(model, newdata=d_predicted, se.fit=TRUE) 
...
# ---- model-B-global ------------------------------
pooled_B <- estimate_pooled_model(ds2, predictors_B)
summary(pooled_B)

# ---- model-B-local ------------------------------
local_B <- estimate_local_models(ds2, predictors_B)
sapply(local_B, summary)
lapply(local_B, summary)

# ---- model-C-global ------------------------------
pooled_C <- estimate_pooled_model(ds2, predictors_C)
summary(pooled_C)

# ---- model-C-local ------------------------------
local_C <- estimate_local_models(ds2, predictors_C)
sapply(local_C, summary)
lapply(local_C, summary)


# ---- model-D-global ------------------------------
pooled_D <- estimate_pooled_model(ds2, predictors_D)
summary(pooled_D)

# ---- model-D-local ------------------------------
local_D <- estimate_local_models(ds2, predictors_D)
sapply(local_D, summary)
lapply(local_D, summary)


# ---- model-E-global ------------------------------
pooled_E <- estimate_pooled_model(ds2, predictors_E)
summary(pooled_E)

# ---- model-E-local ------------------------------
local_E <- estimate_local_models(ds2, predictors_E)
sapply(local_E, summary)
lapply(local_E, summary)


# ---- model-F-global ------------------------------
pooled_F <- estimate_pooled_model(ds2, predictors_F)
summary(pooled_F)

# ---- model-F-local ------------------------------
local_F <- estimate_local_models(ds2, predictors_F)
sapply(local_F, summary)
lapply(local_F, summary)


# ---- model-G-global ------------------------------
pooled_G <- estimate_pooled_model(ds2, predictors_G)
summary(pooled_G)

# ---- model-G-local ------------------------------
local_G <- estimate_local_models(ds2, predictors_G)
sapply(local_G, summary)
lapply(local_G, summary)
