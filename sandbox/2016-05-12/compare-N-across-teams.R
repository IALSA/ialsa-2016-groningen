...
# knitr::stitch_rmd(script="./sandbox/2016-05-12/compare-N-across-teams.R", output="./sandbox/2016-05-12/stitched-output/compare-N-across-teams.md")
...
assemble_dto <- function(dto, get_these_variables){
  ...
  list_of_harmonized_ds <- list() #  list object with harmonized data
  for(s in dto[["studyName"]]){
    ds <- dto[["unitData"]][[s]] # get study data from dto
    variables_present <- colnames(ds) %in% get_these_variables # variables on the list
    list_of_harmonized_ds[[s]] <- ds[, variables_present] # keep only them
  }
  return(list_of_harmonized_ds)
}
list_of_harmonized_ds <- assemble_dto(
  dto=dto,
...
lapply(list_of_harmonized_ds, names) # view the contents of the list object
list_of_harmonized_ds %>% lapply(nrow)
ds <- plyr::ldply(list_of_harmonized_ds,data.frame, .id = "study_name")
...
names(dto$unitData$share)  

t <- ds %>% 
  dplyr::filter(study_name=="share") %>% 
  dplyr::group_by(age_in_years) %>% 
  dplyr::summarize(count=n())
print(t, n=nrow(t))
...
# d <- ds %>%  dplyr::filter(study_name=="share")
# t <- table(
#   d$year_born, 
#   cut(d$age_in_years,breaks = c(-Inf,seq(from=-25,to=30,by=5), Inf)),
#   
#   useNA = "always"
# ); t[t==0] <- "."; t
