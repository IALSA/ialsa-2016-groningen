...
d <- ds %>% na.omit()
mdl <- glm(
  # formula = smoke_now ~ 1 + study_name + age_in_years + sex + marital ,
...
# graph
g <- ggplot2::ggplot(d, aes_string(x="age_in_years")) +
  geom_point(aes_string(y="smoke_now_p", color="sex"), shape=21) +
  # geom_line(aes_string(y="smoke_now_p", group="sex", color="sex")) +
