...
read_chunk("./reports/harmonize-physique/harmonize-physique.R") 
```
...
Items that can contribute to generating values for the harmonized variable `bmi`  are:
```{r II-B-physique-satsa-1, echo=TRUE}
```
We compute `bmi` according to the declared formula:  
```{r II-B-physique-satsa-2, echo=TRUE}
```

### SHARE

Items that can contribute to generating values for the harmonized variable `bmi`  are:
```{r II-B-physique-share-1, echo=TRUE}
```
We compute `bmi` according to the declared formula:  
```{r II-B-physique-share-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `bmi`  are:
```{r II-B-physique-tilda-1, echo=TRUE}
```
We compute `bmi` according to the declared formula:  
```{r II-B-physique-tilda-2, echo=TRUE}
```
