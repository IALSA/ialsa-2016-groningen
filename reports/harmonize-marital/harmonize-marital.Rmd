...
read_chunk("./reports/harmonize-marital/harmonize-marital.R") 
```
...
```{r II-B-marital-alsa-1, echo=TRUE}
```
...
```{r II-B-marital-alsa-2, echo=TRUE}
```


### LBSL
Items that can contribute to generating values for the harmonized variable `marital`  are:
```{r II-B-marital-lbsl-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-marital-lbsl-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `marital`  are:
```{r II-B-marital-satsa-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-marital-satsa-2, echo=TRUE}
```

### SHARE

Items that can contribute to generating values for the harmonized variable `marital`  are:
```{r II-B-marital-share-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-marital-share-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `marital`  are:
```{r II-B-marital-tilda-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-marital-tilda-2, echo=TRUE}
```
...
```{r II-C-marital-alsa-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-C-marital-alsa-2, echo=TRUE}
```


### LBLS
Items that can contribute to generating values for the harmonized variable `single`  are:
```{r II-C-marital-lbsl-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-C-marital-lbsl-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `single`  are:
```{r II-C-marital-satsa-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-C-marital-satsa-2, echo=TRUE}
```

### SHARE

Items that can contribute to generating values for the harmonized variable `single`  are:
```{r II-C-marital-share-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-C-marital-share-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `single`  are:
```{r II-C-marital-tilda-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-C-marital-tilda-2, echo=TRUE}
```
