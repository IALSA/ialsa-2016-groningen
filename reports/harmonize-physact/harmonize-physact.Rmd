---
title: "Harmonize: physical activity"
output:
  html_document:
...
read_chunk("./reports/harmonize-physact/harmonize-physact.R") 
```
...
```{r II-B-physact-alsa-1, echo=TRUE}
```
...
```{r II-B-physact-alsa-2, echo=TRUE}
```


### LBSL
Items that can contribute to generating values for the harmonized variable `sedentary`  are:
```{r II-B-physact-lbsl-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-physact-lbsl-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `sedentary`  are:
```{r II-B-physact-satsa-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-physact-satsa-2, echo=TRUE}
```

### SHARE

Items that can contribute to generating values for the harmonized variable `sedentary`  are:
```{r II-B-physact-share-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-physact-share-2, echo=TRUE}
```
...
Items that can contribute to generating values for the harmonized variable `sedentary`  are:
```{r II-B-physact-tilda-1, echo=TRUE}
```
We encode the harmonization rule by manually editing the values in a corresponding `.csv` file located in  `./data/meta/h-rules/`. Then, we apply the recoding logic it contains and append the newly created, harmonized variable to the initial data set. 
```{r II-B-physact-tilda-2, echo=TRUE}
```
